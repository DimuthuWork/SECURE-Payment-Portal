...
let transporter;
let emailFromAddress = process.env.EMAIL_USER;

async function setupEmailTransporter() {
    // Use .env credentials when real SMTP is configured (Gmail, etc.)
    const useRealEmail = process.env.EMAIL_HOST && 
        !process.env.EMAIL_HOST.includes('ethereal') &&
    ...
    if (useRealEmail) {
        ...
        // Use Ethereal for development/testing - emails won't reach real inboxes
        const testAccount = await nodemailer.createTestAccount();
        emailFromAddress = testAccount.user;
        transporter = nodemailer.createTransport({
        ...
        console.log('📧 Using Ethereal test account (emails do NOT reach real inboxes - check console for preview URL)');
    }
}
...
app.post('/api/generate-invoice', async (req, res) => {
    try {
        ...
        const mailOptions = {
            from: `"${process.env.EMAIL_FROM_NAME || 'Ultra Secure Payment Portal'}" <${emailFromAddress}>`,
            to: customerEmail,
        ...
        // Log test email URL when using Ethereal (emails don't reach real inboxes)
        const previewUrl = nodemailer.getTestMessageUrl(info);
        if (previewUrl) {
            console.log('✅ OTP email sent! Ethereal preview URL:', previewUrl);
        } else {
        ...
        res.json({
            ...
            data: {
                ...
                // In development, also return OTP so the flow works without real email
